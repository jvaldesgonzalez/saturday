      ...
      this.persistenceManager.query<ISearchResultItem<AccountItem>>(
        QuerySpecification.withStatement(
          `
				...
				'optional match (a)-[r]-(u:User) where u.id = uId return a{.fullname, .username, .id, .avatar, type:"user", friendshipStatus: case type(r) case r is null then 'none' else toLower(type(r)) end, privacy:"public"} as result',
